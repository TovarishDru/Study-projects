...
int main() {
    try {
        ...
        cin >> a >> n >> b >> e;
        cout << b;
        JacobiAlphaMatrix alpha(a); 
        JacobiBetaVector beta(a, b);
        ColumnVector x(beta);
        ColumnVector xPrev(x);
        SquareMatrix lower = getLowerPart(alpha);
        SquareMatrix upper = alpha - lower;
        cout << "alpha:\n" << alpha;
        cout << "beta:\n" << beta;
        cout << "B:\n" << lower;
        cout << "C:\n" << upper;
        SquareMatrix mult = IdentityMatrix(n) - lower;
        cout << "I-B:\n" << mult;
        mult = findInverse(mult);
        cout << "(I-B)_-1:" << mult;
        int step = 1;
        double cur_e;
        do {
            xPrev = x;
            x = mult * upper * xPrev + mult * beta;
            cout << "x(" << step++ << "):\n";
            cout << x;
            cur_e = abs(ColumnVector(x - xPrev).norm());


            string num = to_string(round(cur_e * 10000) / 10000);
            cout << "e: ";
            for (int k = 0; k < num.size(); k++) {
                if (num[k] == '.') {
                    cout << num[k] << num[k + 1] << num[k + 2] << num[k + 3] << num[k + 4];
                    break;
                }
                cout << num[k];
            }
