...
run_tests(){
    ...
    while IFS= read -r line; do
        ...
        name=$(echo "$line" | cut -d ']' -f 2| awk -F ':' '{print $1}' | xargs)
        type=$(echo "$line" | cut -d ']' -f 2| awk -F ':' '{print $2}' | xargs)
        number=$(echo "$line" | cut -d ']' -f 2| awk -F ':' '{print $3}' | xargs)
    ...
    if (($errors == 0)); then
        if (($MESSAGE_COUNT == ${#original_numbers[@]} && $MESSAGE_COUNT == ${#squared_numbers[@]} && $MESSAGE_COUNT == ${#cubed_numbers[@]})); then
            echo "SUCCESS: All records match correctly!"
            exit 0
        else
            if (($MESSAGE_COUNT != ${#original_numbers[@]})); then
                echo "FAILURE: messages sent: $MESSAGE_COUNT, original numbers logged: ${#original_numbers[@]}]"
            fi
            if (($MESSAGE_COUNT != ${#squared_numbers[@]})); then
                echo "FAILURE: messages sent: $MESSAGE_COUNT, squared numbers logged: ${#squared_numbers[@]}]"
            fi
            if (($MESSAGE_COUNT != ${#cubed_numbers[@]})); then
                echo "FAILURE: messages sent: $MESSAGE_COUNT, cubed numbers logged: ${#cubed_numbers[@]}]"
            fi
            exit 1
        fi
    else
...
start_service "Logger" "applications/Logger.py"
